...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  var connector = dataSource.connector = new MongoDB(s, dataSource);
  dataSource.ObjectID = mongodb.ObjectID;
    ...
    DataAccessObject.textSearch = function (filter, text, options, callback) {
        return connector.textSearch(this.modelName, filter, text, options, callback);
    };
 ...
 */
MongoDB.prototype.textSearch = function (model, filter, text, options, callback) {
    var self = this;
    if (self.debug) {
        debug('all', model, filter);
    }
    ...
    if (fields) {
        this.execute(model, 'find', query, textScore, fields, processResponse);
    } else {
        this.execute(model, 'find', query, textScore, processResponse);
    }
